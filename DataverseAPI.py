...
class DataverseAPI:
    ...
    def _get_cache_filename(self, endpoint):
        ...
        return f"homepagebuilder/cache_{encoded_name}.json"
    ...
    def get_extended_subdataverse_info(self, subdataverse):
        ...
        if self.check_image_exists(self.base_url + f"/homepage/img/backgrounds/collection_{dataverse_details['data']['alias']}.jpg"):
        ...
        if subdataverses and 'data' in subdataverses:
            for subdataverse in subdataverses['data']:
                if 'type' in subdataverse and subdataverse['type'] == 'dataverse':
                    ...
                    if fullinfo["dataverseType"]=="RESEARCHER":
                        continue
    ...
    def parse_datasets_for_carousel(self, base_dataverse, limit):
        ...
        # Parse the JSON data
        json_response=self._make_request(f"search?sort=date&order=desc&q=*&per_page={limit}&type=dataset&subtree={base_dataverse}")

        if json_response.get('status') == 'OK' and 'items' in json_response.get('data', {}):
            for item in json_response['data']['items']:
                # Extracting necessary fields
                dataset_info = {
                    'url': item.get('url', '#'),
                    'img_src': 'path/to/default/image.png',  # Replace with actual logic for image source
                    'img_alt': item.get('name', 'No name provided'),
                    'title': item.get('dataset_name', 'No title provided'),
                    'date': item.get('published_at', 'No date provided'),
                    'authors': item.get('authors', []),
                    'description': item.get('description', 'No description provided'),
                    'name_of_dataverse':item.get('name_of_dataverse', 'bonndata'),

                }
                #extendedinfo=self.list_files_in_dataset(item['global_id'])
    ...
    def dataset_info(self, doi):
        ...
        #info= self._make_request(endpoint)
        #info['data']['latestVersion']["identifier_of_dataverse"]=info['data']["publisher"]
        #info['data']['latestVersion']['global_id'] = info['data']['latestVersion']["datasetPersistentId"]
        #return info['data']['latestVersion']
    ...
    def dataset_statistics(self, dataset):
        ...
        if 'downloadsUnique' in dataset:
            ...
            if "data" in downloads and 'downloadsUnique' in downloads['data']:
        ...
        if "data" in views and 'viewsUnique' in views['data']:
    ...
    def calculateMetrics(self, base_dataverse):
        ...
        now = datetime.now()
        # Format for the current month
        current_month = now.strftime("%Y-%m")

        delta=timedelta(days=32)
        first_day_of_next_month = datetime(now.year, now.month, 1) + delta
        # Then, subtract one day to find the last day of the current month
        ...
        # Previous month: Subtract 1 from the month, handle year change if necessary
        if now.month == 1:
        ...
        #prevcnt=(self.sumup3(prev))
        #curcnt=(self.sumup3(cur))
        ...
        for csvrespons in [prev, cur
            ...
            for row in csvrespons:
                ...
                if row[0] in index:
                    count=index[row[0]]
                #if count>0:
                #    count=int(count*days_until_end_of_month/30)
    ...
    def uniqueDownloads(self, base_dataverse, current_month):
        #endpoint = f"info/metrics/filedownloads/toMonth/{current_month}/?parentAlias={base_dataverse}"
    ...
    def get_dataset_id_from_doi(self, doi):
        ...
        :return: The dataset ID if found, otherwise None.
        """
        # The endpoint to get dataset by its DOI
        endpoint = f"datasets/:persistentId/?persistentId={doi}"
        response = self._make_request(endpoint)
        if response and 'data' in response:
            # The dataset ID is usually found in the response data under the datasetPersistentId key
            dataset_id = response['data'].get('id', None)
    ...
    def get_dataset_citation_image_src(self, persistent_id):
        ...
        try:
            ...
            # Find the div with class 'preview-icon-block' within the element with the specified id
            ...
            # Find the img within this div and get its src attribute
    ...
    def list_files_in_dataset(self, persistent_id):
        ...
        if response and 'data' in response and 'latestVersion' in response['data']:
            ...
            return [{'fileName': file['dataFile']['filename'], 'fileId': file['dataFile']['id']} for file in files_info]
    ...
    def get_usage_statistics(self, object_id, object_type='dataverse'):
        ...
        """
        metrics_to_fetch = ['views', 'downloads']  # Define which metrics you want to fetch.
        statistics = {}
    ...
    def get_published_datasets(self,base_dataverse):
        #endpoint = f"info/metrics/datasets"
    ...
    def get_downloads_last_days(self,base_dataverse,dayscnt=30):
        ...
        #endpoint = f"info/metrics/downloads/pastDays/{dayscnt}"
        stats_response = self._make_request(endpoint)
